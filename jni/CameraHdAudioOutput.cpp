...
//#define LOG_NDEBUG 0
#define LOG_TAG "CameraHdAudioOutput"
...
#include <media/CameraHdAudioOutput.h>
#include <utils/Log.h>
#include <private/media/CameraHdAudioOutputShared.h>
...
// static
status_t CameraHdAudioOutput::getMinFrameCount(
        size_t* frameCount,
        ...
        uint32_t sampleRate)
{
    ...
    *frameCount = (sampleRate == 0) ? afFrameCount * minBufCount :
    ...
    if (*frameCount == 0) {
        ALOGE("CameraHdAudioOutput::getMinFrameCount failed for streamType %d, sampleRate %d",
...
CameraHdAudioOutput::CameraHdAudioOutput()
    : mStatus(NO_INIT),
      mIsTimed(false),
...
CameraHdAudioOutput::~CameraHdAudioOutput()
{
    if (mStatus == NO_ERROR) {
        ...
        stop();
        if (mCameraHdAudioOutputThread != 0) {
            mProxy->interrupt();
            mCameraHdAudioOutputThread->requestExit();   // see comment in CameraHdAudioOutput.h
            mCameraHdAudioOutputThread->requestExitAndWait();
            mCameraHdAudioOutputThread.clear();
        }
        IInterface::asBinder(mCameraHdAudioOutput)->unlinkToDeath(mDeathNotifier, this);
        mCameraHdAudioOutput.clear();
        mCblkMemory.clear();
        mSharedBuffer.clear();
        IPCThreadState::self()->flushCommands();
        ALOGV("~CameraHdAudioOutput, releasing session id from %d on behalf of %d",
                IPCThreadState::self()->getCallingPid(), mClientPid);
        AudioSystem::releaseAudioSessionId(mSessionId, mClientPid);
    }
}

status_t CameraHdAudioOutput::set(
        audio_stream_type_t streamType,
        uint32_t sampleRate,
        audio_format_t format,
        audio_channel_mask_t channelMask,
        ...
        const audio_attributes_t* pAttributes)
{
    ...
    // invariant that mCameraHdAudioOutput != 0 is true only after set() returns successfully
    if (mCameraHdAudioOutput != 0) {
    ...
    if (pAttributes == NULL) {
        ...
        ALOGV("Building CameraHdAudioOutput with attributes: usage=%d content=%d flags=0x%x tags=[%s]",
    ...
    if (cbf != NULL) {
        mCameraHdAudioOutputThread = new CameraHdAudioOutputThread(*this, threadCanCallJava);
        mCameraHdAudioOutputThread->run("CameraHdAudioOutput", ANDROID_PRIORITY_AUDIO, 0 /*stack*/);
    }

    // create the ICameraHdAudioOutput
...
status_t CameraHdAudioOutput::start()
{
    AutoMutex lock(mLock);
    ...
    sp<CameraHdAudioOutputThread> t = mCameraHdAudioOutputThread;
    if (t != 0) {
    ...
    if (!(flags & CBLK_INVALID)) {
        status = mCameraHdAudioOutput->start();
...
bool CameraHdAudioOutput::stopped() const
{
    AutoMutex lock(mLock);
    return mState != STATE_ACTIVE;
}

void CameraHdAudioOutput::flush()
{
    if (mSharedBuffer != 0) {
        return;
    }
...
void CameraHdAudioOutput::flush_l()
{
    ...
    mProxy->flush();
    mCameraHdAudioOutput->flush();
}

void CameraHdAudioOutput::pause()
{
    AutoMutex lock(mLock);
    if (mState == STATE_ACTIVE) {
        mState = STATE_PAUSED;
    ...
    if (isOffloaded_l()) {
        if (mOutput != AUDIO_IO_HANDLE_NONE) {
            ...
            ALOGV("CameraHdAudioOutput::pause for offload, cache current position %u", mPausedPosition);
        }
    }
}

status_t CameraHdAudioOutput::setVolume(float left, float right)
{
    // This duplicates a test by CameraHdAudioOutput JNI, but that is not the only caller
    ...
    if (isOffloaded_l()) {
        mCameraHdAudioOutput->signal();
    }
    return NO_ERROR;
}

status_t CameraHdAudioOutput::setVolume(float volume)
{
    return setVolume(volume, volume);
}

status_t CameraHdAudioOutput::setAuxEffectSendLevel(float level)
{
    // This duplicates a test by CameraHdAudioOutput JNI, but that is not the only caller
...
void CameraHdAudioOutput::getAuxEffectSendLevel(float* level) const
{
    if (level != NULL) {
        *level = mSendLevel;
    }
...
status_t CameraHdAudioOutput::setSampleRate(uint32_t rate)
{
...
uint32_t CameraHdAudioOutput::getSampleRate() const
{
    if (mIsTimed) {
        return 0;
    }
...
status_t CameraHdAudioOutput::setLoop(uint32_t loopStart, uint32_t loopEnd, int loopCount)
{
...
void CameraHdAudioOutput::setLoop_l(uint32_t loopStart, uint32_t loopEnd, int loopCount)
{
...
status_t CameraHdAudioOutput::setMarkerPosition(uint32_t marker)
{
...
status_t CameraHdAudioOutput::getMarkerPosition(uint32_t *marker) const
{
...
status_t CameraHdAudioOutput::setPositionUpdatePeriod(uint32_t updatePeriod)
{
...
status_t CameraHdAudioOutput::getPositionUpdatePeriod(uint32_t *updatePeriod) const
{
    if (isOffloadedOrDirect()) {
        return INVALID_OPERATION;
    }
...
status_t CameraHdAudioOutput::setPosition(uint32_t position)
{
    if (mSharedBuffer == 0 || mIsTimed || isOffloadedOrDirect()) {
        return INVALID_OPERATION;
    }
...
status_t CameraHdAudioOutput::getPosition(uint32_t *position)
{
    if (position == NULL) {
        return BAD_VALUE;
    }
    ...
    if (isOffloadedOrDirect_l()) {
        ...
        *position = dspFrames;
        ...
        // ICameraHdAudioOutput::stop() isn't synchronous; we don't know when presentation completes
...
status_t CameraHdAudioOutput::getBufferPosition(uint32_t *position)
{
...
status_t CameraHdAudioOutput::reload()
{
    if (mSharedBuffer == 0 || mIsTimed || isOffloadedOrDirect()) {
        return INVALID_OPERATION;
    }
...
audio_io_handle_t CameraHdAudioOutput::getOutput() const
{
    AutoMutex lock(mLock);
    return mOutput;
}

status_t CameraHdAudioOutput::attachAuxEffect(int effectId)
{
    AutoMutex lock(mLock);
    status_t status = mCameraHdAudioOutput->attachAuxEffect(effectId);
    if (status == NO_ERROR) {
...
audio_stream_type_t CameraHdAudioOutput::streamType() const
{
...
status_t CameraHdAudioOutput::createTrack_l()
{
    ...
    if ((mFlags & AUDIO_OUTPUT_FLAG_FAST) && !((
        ...
        // once denied, do not request again if ICameraHdAudioOutput is re-created
        mFlags = (audio_output_flags_t) (mFlags & ~AUDIO_OUTPUT_FLAG_FAST);
    }
    ALOGV("createTrack_l() output %d afLatency %d", output, afLatency);

    // The client's CameraHdAudioOutput buffer is divided into n parts for purpose of wakeup by server, where
    ...
    if (!audio_is_linear_pcm(mFormat)) {
        ...
        // When initializing a shared buffer CameraHdAudioOutput via constructors,
        // there's no frameCount parameter.
        // But when initializing a shared buffer CameraHdAudioOutput via set(),
    ...
    if (mFlags & AUDIO_OUTPUT_FLAG_FAST) {
        ...
        if (mCameraHdAudioOutputThread != 0) {
            tid = mCameraHdAudioOutputThread->getTid();
        }
    }
    ...
    sp<ICameraHdAudioOutput> track = audioFlinger->createTrack(streamType,
                                                      mSampleRate,
    ...
    if (temp < frameCount || (frameCount == 0 && temp == 0)) {
        // In current design, CameraHdAudioOutput client checks and ensures frame count validity before
    ...
    mCameraHdAudioOutput->attachAuxEffect(mAuxEffectId);
    // FIXME don't believe this lie
    mLatency = afLatency + (1000*frameCount) / mSampleRate;

    mFrameCount = frameCount;
    // If ICameraHdAudioOutput is re-created, don't let the requested frameCount
    ...
    if (mSharedBuffer == 0) {
        mStaticProxy.clear();
        mProxy = new CameraHdAudioOutputClientProxy(cblk, buffers, frameCount, mFrameSizeAF);
    } else {
        mStaticProxy = new StaticCameraHdAudioOutputClientProxy(cblk, buffers, frameCount, mFrameSizeAF);
        mProxy = mStaticProxy;
    }
    ...
    IInterface::asBinder(mCameraHdAudioOutput)->linkToDeath(mDeathNotifier, this);
...
status_t CameraHdAudioOutput::obtainBuffer(Buffer* audioBuffer, int32_t waitCount)
{
    if (audioBuffer == NULL) {
        return BAD_VALUE;
    }
...
status_t CameraHdAudioOutput::obtainBuffer(Buffer* audioBuffer, const struct timespec *requested,
        struct timespec *elapsed, size_t *nonContig)
{
    // previous and new ICameraHdAudioOutput sequence numbers are used to detect track re-creation
    ...
    do {
        ...
        sp<CameraHdAudioOutputClientProxy> proxy;
        sp<IMemory> iMem;
...
void CameraHdAudioOutput::releaseBuffer(Buffer* audioBuffer)
{
...
ssize_t CameraHdAudioOutput::write(const void* buffer, size_t userSize, bool blocking)
{
    ...
    if (ssize_t(userSize) < 0 || (buffer == NULL && userSize != 0)) {
        ...
        ALOGE("CameraHdAudioOutput::write(buffer=%p, size=%zu (%zd)", buffer, userSize, userSize);
        return BAD_VALUE;
    }
...
TimedCameraHdAudioOutput::TimedCameraHdAudioOutput() {
    mIsTimed = true;
}

status_t TimedCameraHdAudioOutput::allocateTimedBuffer(size_t size, sp<IMemory>* buffer)
{
    AutoMutex lock(mLock);
    status_t result = UNKNOWN_ERROR;

#if 1
    // acquire a strong reference on the IMemory and ICameraHdAudioOutput so that they cannot be destroyed
    // while we are accessing the cblk
    sp<ICameraHdAudioOutput> CameraHdAudioOutput = mCameraHdAudioOutput;
    ...
    if (!(cblk->mFlags & CBLK_INVALID)) {
        result = mCameraHdAudioOutput->allocateTimedBuffer(size, buffer);
...
status_t TimedCameraHdAudioOutput::queueTimedBuffer(const sp<IMemory>& buffer,
                                           int64_t pts)
{
    status_t status = mCameraHdAudioOutput->queueTimedBuffer(buffer, pts);
    {
        AutoMutex lock(mLock);
        audio_track_cblk_t* cblk = mCblk;
        // restart track if it was disabled by audioflinger due to previous underrun
...
status_t TimedCameraHdAudioOutput::setMediaTimeTransform(const LinearTransform& xform,
                                                TargetTimeline target)
{
    return mCameraHdAudioOutput->setMediaTimeTransform(xform, target);
}

// -------------------------------------------------------------------------

nsecs_t CameraHdAudioOutput::processAudioBuffer()
{
    // Currently the CameraHdAudioOutput thread is not created if there are no callbacks.
    ...
    if (flags & CBLK_INVALID) {
        ...
        if (!isOffloadedOrDirect_l() || (mSequence == mObservedSequence)) {
            ...
            mLock.unlock();
            // Run again immediately, but with a new ICameraHdAudioOutput
            return 0;
        }
    }
    ...
    sp<CameraHdAudioOutputClientProxy> proxy = mProxy;
    ...
    if (waitStreamEnd) {
        ...
        case TIMED_OUT:
            mCbf(EVENT_STREAM_END, mUserData, NULL);
            {
                ...
                // or another thread could have re-started the CameraHdAudioOutput during that time.
    ...
    if (mObservedSequence != sequence) {
        ...
        mCbf(EVENT_NEW_ICameraHdAudioOutput, mUserData, NULL);
...
status_t CameraHdAudioOutput::restoreTrack_l(const char *from)
{
    ALOGW("dead ICameraHdAudioOutput, %s, creating a new one from %s()",
    ...
    // If a new ICameraHdAudioOutput is successfully created, createTrack_l() will modify the
    // following member variables: mCameraHdAudioOutput, mCblkMemory and mCblk.
    // It will also delete the strong references on previous ICameraHdAudioOutput and IMemory.
    // If a new ICameraHdAudioOutput cannot be created, the previous (dead) instance will be left intact.
...
#endif
        if (mState == STATE_ACTIVE) {
            result = mCameraHdAudioOutput->start();
        }
    }
    if (result != NO_ERROR) {
...
uint32_t CameraHdAudioOutput::updateAndGetPosition_l()
{
...
status_t CameraHdAudioOutput::setParameters(const String8& keyValuePairs)
{
    AutoMutex lock(mLock);
    return mCameraHdAudioOutput->setParameters(keyValuePairs);
}

status_t CameraHdAudioOutput::getTimestamp(AudioTimestamp& timestamp)
{
    AutoMutex lock(mLock);
    ...
    status_t status = mCameraHdAudioOutput->getTimestamp(timestamp);
    if (status != NO_ERROR) {
    ...
    if (isOffloadedOrDirect_l()) {
        ...
        // ICameraHdAudioOutput.  And timestamp.mPosition is initially in server's
        // point of view, so we need to apply the same fudge factor to it.
    }
    return status;
}

String8 CameraHdAudioOutput::getParameters(const String8& keys)
{
...
bool CameraHdAudioOutput::isOffloaded() const
{
    AutoMutex lock(mLock);
    return isOffloaded_l();
}

bool CameraHdAudioOutput::isDirect() const
{
    AutoMutex lock(mLock);
    return isDirect_l();
}

bool CameraHdAudioOutput::isOffloadedOrDirect() const
{
    AutoMutex lock(mLock);
...
status_t CameraHdAudioOutput::dump(int fd, const Vector<String16>& args __unused) const
{
    ...
    result.append(" CameraHdAudioOutput::dump\n");
...
uint32_t CameraHdAudioOutput::getUnderrunFrames() const
{
    AutoMutex lock(mLock);
...
void CameraHdAudioOutput::DeathNotifier::binderDied(const wp<IBinder>& who __unused)
{
    sp<CameraHdAudioOutput> CameraHdAudioOutput = mCameraHdAudioOutput.promote();
    if (CameraHdAudioOutput != 0) {
        AutoMutex lock(CameraHdAudioOutput->mLock);
        CameraHdAudioOutput->mProxy->binderDied();
    }
}

// =========================================================================

CameraHdAudioOutput::CameraHdAudioOutputThread::CameraHdAudioOutputThread(CameraHdAudioOutput& receiver, bool bCanCallJava)
    : Thread(bCanCallJava), mReceiver(receiver), mPaused(true), mPausedInt(false), mPausedNs(0LL),
      mIgnoreNextPausedInt(false)
{
}

CameraHdAudioOutput::CameraHdAudioOutputThread::~CameraHdAudioOutputThread()
{
}

bool CameraHdAudioOutput::CameraHdAudioOutputThread::threadLoop()
{
    {
...
void CameraHdAudioOutput::CameraHdAudioOutputThread::requestExit()
{
...
void CameraHdAudioOutput::CameraHdAudioOutputThread::pause()
{
    AutoMutex _l(mMyLock);
    mPaused = true;
}

void CameraHdAudioOutput::CameraHdAudioOutputThread::resume()
{
    AutoMutex _l(mMyLock);
...
void CameraHdAudioOutput::CameraHdAudioOutputThread::pauseInternal(nsecs_t ns)
{
    AutoMutex _l(mMyLock);
    mPausedInt = true;
    mPausedNs = ns;
